...
form_buttons = [
    sg.HorizontalSeparator(pad=20, color=(173, 204, 218)),
    reset,
    sg.HorizontalSeparator(color=(173, 204, 218)),
    submit,
    sg.HorizontalSeparator(pad=(160, 0), color=(173, 204, 218)),
    help,
    sg.HorizontalSeparator(pad=(1, 0), color=(173, 204, 218)),
]
...
output_checkbox = [
    sg.Checkbox(
        "Output to individual part folders", font=h3, key="checkbox", default=True
    )
]
split_setting = [
    sg.Text("Split by"),
    sg.Radio("Bookmarks", default=True, key="split_bookmarks", group_id=1),
    sg.Radio("Pages", key="split_pages", default=False, group_id=1),
]
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        sg.popup_scrolled(
            help_text, title="Score Splitter Instructions", font=h3, size=(50, 10)
        )
        ...
        try:
            ...
            parts = (
                convert_string_to_array(values["part_names"])
                if values["split_bookmarks"] == True
                else convert_string_to_part_pages(values["part_names"]).keys()
            )
            ...
            if values["split_bookmarks"] == True:
                ...
                split_score_by_pages(
                    score_path,
                    convert_string_to_part_pages(values["part_names"]),
                    score_metadata,
                    output_path,
                )
